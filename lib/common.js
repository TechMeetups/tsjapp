...
job_manager =
{
  ...
  default_subscribe : function(event_id,company_id)
  {
    ...
    return Meteor.subscribe('jobs',Session.get("job_limit"),event_id,company_id);
  },
  format_data : function(date)
  {
    return moment(date).format("DD/MM/YYYY");
...
event_manager =
{
    ...
    getList : function ()
    {
      if (Session.get('search_terms'))
        return Events.find({'name': {'$regex': new RegExp(Session.get('search_terms'), "i")}},{sort:{ start:1}});
      else
