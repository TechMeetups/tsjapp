...
{
    console.log('match_get_job_skill:'+job.title) ;
    ...
    {
        var map = KeywordMap.findOne({keyword:words[i],class:'skill'}) ; 
        if(map)
        {
            //Job.update({_id:job._id},{ $set: {skill:map.word} } )  ; 
            job.skill = map.word ; 
            console.log('Mapped :'+words[i]+' -->'+map.word) ; 
...
{
    console.log('match_get_job_profession:'+job.title) ;
    var title = job.title.toLowerCase() ; 
    var words = title.split(/[\s,]+/) ; 
    console.log(words) ; 
    ...
    for(i=0;i<words.length;i++)
    {
        var map = KeywordMap.findOne({keyword:words[i],class:'profession'}) ; 
        if(map)
        {
            //Job.update({_id:job._id},{ $set: {skill:map.word} } )  ; 
            job.profession = map.word ; 
            console.log('Mapped :'+words[i]+' -->'+map.word) ; 
        }  
    }  
} 