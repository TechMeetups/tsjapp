...
company_manager = {
  ...
  getList : function (event_id)
  {
    company_manager.default_subscribe(event_id);
  ...
  delete : function(company_id){
    Meteor.call('delete_company',company_id,function(e){
      if(e){
        alert(e)
      }else{
        document.location.reload(true);
  ...
  delete_event_company : function(company_id){
    Meteor.call('delete_event_company',company_id,function(e){
      if(e){
        alert(e)
      }else{
        console.log(Router.current().params._id)
      }
      Session.set('company_terms','abc');
...
attendee_manager =
{
  ...
  getList : function (event_id)
  {
    ...
    {
        return Meteor.users.find({}, {sort:{ 'profile.premium' : -1 }}) ;
    }
  ...
  updatePriority: function(user_id)
  {
    return Meteor.call('update_attendee_priority',user_id);
  },
  search : function(search_terms)
  {
...
match_manager =
{
    ...
    email_matched_job : function( title, profession, experience, skill, city   )
    {
        // match_manager.loadKeyWords() ;
        ...
        var usr = Meteor.user() ;
         ...
         IonLoading.show(
        {
          customTemplate: "Emaling matched Job report...",
          duration: 2000
        });
...
match_user_job = function(usr,job)
{
  ...
  // console.log(usr) ;
    ...
    if( !usr || !usr.profile )
    {
      console.log('User doesnt have a profile')
      console.log(usr) ; 
      return false ; 
    } 
