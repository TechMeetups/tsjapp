...
match_user_job = function(usr,job)
{
  ...
  {
    var user_skill = usr.profile.skill.split(',') ; 
    var counter = 0 ; 

    for(x=0;x<user_skill.length;x++)
    {
        if( job.title.toLowerCase().indexOf(user_skill[x].toLowerCase()) > -1)
        {
            ++counter ; 
        }  

        if( job.desc.toLowerCase().indexOf(user_skill[x].toLowerCase()) > -1)
        {
            ++counter ; 
        }  
    }
